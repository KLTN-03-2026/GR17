      ...
      <form class="partner-modal" @submit.prevent="saveForm">
        ...
        <section class="partner-modal__body">
          ...
          <div class="field-grid">
            <label class="field">
              <span>Giá (đ)</span>
              <input v-model.number="form.so_tien" type="number" min="0">
            </label>
            <label class="field">
              <span>Số ngày</span>
  ...
  methods: {
    ...
    async loadPage() {
      ...
      try {
        ...
        this.errorMessage = error?.message || "Không thể tải dữ liệu quản lý tour.";
      } finally {
        this.loading = false;
      }
    },
    ...
    openEditForm(tour) {
      ...
      this.form = {
        ...
        so_tien: tour?.so_tien ? Number(tour.so_tien) : null,
    ...
    normalizePayload() {
      return {
        ...
        so_tien: this.form.so_tien || null,
    ...
    async submitTour(tour) {
      ...
      try {
        ...
        await showAlert({
          title: "Đã gửi duyệt",
          message: "Tour đã được gửi duyệt thành công.",
          tone: "success",
        });
        await this.loadPage();
      } catch (error) {
        await showAlert({
          ...
          message: error?.message || "Không thể gửi duyệt tour.",
          tone: "danger",
        });
      } finally {
    ...
    async deleteTour(tour) {
      ...
      try {
        ...
        await showAlert({
          title: "Xóa tour thất bại",
          message: error?.message || "Không thể xóa tour.",
          tone: "danger",
        });
      } finally {
        this.deletingTourId = "";
