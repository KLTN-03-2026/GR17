    ...
    <div v-if="hien_form_khoi_hanh" class="schedule-create-backdrop" @click.self="dongFormKhoiHanh">
      <form class="schedule-create-modal" @submit.prevent="luuLichKhoiHanh">
        ...
        <div class="schedule-create-modal__grid">
          ...
          <label>
            <span>Số tiền (Giá)</span>
            <input v-model.number="form_khoi_hanh.so_tien" type="number" min="0" required>
          </label>
  ...
  methods: {
    ...
    moFormKhoiHanh() {
      if (!this.danh_sach_tour_options.length) {
        toaster.warning('Chưa có tour để gắn lịch khởi hành. Vui lòng tạo tour trước.');
        return;
      }
    ...
    taoPayloadKhoiHanhMoi() {
      return {
        ...
        so_tien: Number(this.form_khoi_hanh.so_tien) || 0,
    ...
    async loadDanhSachLich() {
      ...
      try {
        ...
        this.danh_sach_lich = [];
        toaster.error(this.layThongBaoLoi(error, 'Không thể tải lịch khởi hành tour.'));
      } finally {
        this.dang_tai = false;
      }
    },
    ...
    async xemChiTietLich(lich) {
      try {
        ...
        toaster.error(this.layThongBaoLoi(error, 'Không thể tải chi tiết lịch khởi hành.'));
      }
    },
    dongChiTiet() {
    ...
    async chinhSuaLich(lich) {
      try {
        ...
        this.form_khoi_hanh = {
          ...
          so_tien: Number(this.layGiaTriDauTien(item, ['so_tien', 'So_tien'], 0)) || 0,
        ...
        toaster.error(this.layThongBaoLoi(error, 'Không thể tải dữ liệu lịch khởi hành để chỉnh sửa.'));
      }
    },
    async xoaLich(lich) {
      ...
      try {
        ...
        await this.loadDanhSachLich();
        toaster.success(`Đã xóa lịch khởi hành ${lich.id}.`);
      } catch (error) {
        toaster.error(this.layThongBaoLoi(error, 'Không thể xóa lịch khởi hành.'));
      }
    },
    themLichKhoiHanh() {
      this.moFormKhoiHanh();
