      ...
      <aside class="profile-card">
        <div v-if="doi_tac_dang_chon">
          ...
          <div class="profile-card__actions" v-else-if="doi_tac_dang_chon.trang_thai_duyet === 'approved'">
            <button :class="['action-button', doi_tac_dang_chon.is_block ? 'action-button--primary' : 'action-button--danger']" type="button" @click="toggleKhoaDoiTac(doi_tac_dang_chon)">
              {{ doi_tac_dang_chon.is_block ? 'Mở khóa tài khoản' : 'Khóa tài khoản' }}
            </button>
          </div>
        </div>
  ...
  methods: {
    ...
    async duyetDoiTac(dt) {
      const confirmed = await showConfirm({
        ...
        message: `Xác nhận DUYỆT đối tác: ${dt.ten_doi_tac}?`,
        tone: "info",
        confirmText: "Duyệt",
      });
      if (!confirmed) return;
    ...
    async toggleKhoaDoiTac(dt) {
      const isLocking = !dt.is_block;
      const actionStr = isLocking ? "KHÓA" : "MỞ KHÓA";
      
      const confirmed = await showConfirm({
        title: `Xác nhận ${actionStr.toLowerCase()} tài khoản`,
        message: `Xác nhận ${actionStr} tài khoản: ${dt.ten_doi_tac}?`,
        tone: isLocking ? "danger" : "info",
      ...
      try {
        const res = await goiApi(`/api/admin/doi-tac/${dt.ma_doi_tac}/status`, {
          ...
          body: JSON.stringify({ is_block: isLocking })
        });
        if (res.ok) {
          toaster.success(`Đã ${actionStr.toLowerCase()} đối tác thành công!`);
          this.layDanhSach();
        } else {
          toaster.error(`Lỗi khi ${actionStr.toLowerCase()} đối tác`);
        }
      } catch (err) { toaster.error("Lỗi kết nối"); }
    }
  },
