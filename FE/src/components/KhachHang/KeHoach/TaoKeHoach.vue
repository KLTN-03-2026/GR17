    ...
    <section class="form-shell">
      <div class="form-header">
        <div>
          ...
          <p>Thiết lập thông tin cơ bản, sau đó bạn sẽ chuyển sang màn chỉnh sửa để thêm địa điểm hoặc tạo lịch trình AI.</p>
        </div>
      ...
      <form class="form-card" @submit.prevent="guiBieuMau">
        <div class="form-grid">
          ...
          <label class="field field--full">
            ...
            <input v-model.trim="form.ten_ke_hoach" type="text" placeholder="Ví dụ: Chuyến tham quan miền Bắc">
          ...
          <label class="field">
            ...
            <div class="currency-input-wrapper">
              <input v-model="displayBudget" type="text" placeholder="Ví dụ: 25.000.000" @input="onBudgetInput">
        ...
        <div class="form-actions">
          ...
          <button class="primary-btn" type="submit" :disabled="isSubmitting">
            ...
            <span>{{ isSubmitting ? "Đang lưu..." : "Tạo và chỉnh sửa" }}</span>
          </button>
        </div>
      </form>
    </section>
...
export default {
  ...
  methods: {
    syncDisplayBudget(val) {
      ...
      this.displayBudget = Number.isNaN(num) ? "" : num.toLocaleString("vi-VN");
    },
    onBudgetInput(e) {
    ...
    async taiNhom() {
      try {
        ...
        this.form.ma_nhom = this.form.ma_nhom || this.groups[0]?.id || "";
      } catch (error) {
        this.thongBaoLoi = error.message || "Không thể tải danh sách nhóm.";
      }
    },
    async guiBieuMau() {
      this.errors = validatePlanForm(this.form, { requireCode: false });
      this.thongBaoLoi = "";
      ...
      try {
        ...
        window.setTimeout(() => {
          this.$router.push({ path: `/khach-hang/ke-hoach/${newId}/edit` });
        }, 700);
      } catch (error) {
...
.primary-btn {
  ...
  background: #00476b;
  color: #ffffff;
  box-shadow: 0 14px 28px rgba(15, 79, 115, 0.2);
}
