...
export default {
  ...
  methods: {
    syncSession() {
      const currentRole = authStorage.getCurrentRoleFromPath(this.$route.path);
      
      // Try to get session for current role
      let token = authStorage.getToken(currentRole);
      let user = authStorage.getUser(currentRole);
      let authType = currentRole;

      // Fallback if not specifically in a role path, try to find any active session
      if (!token) {
        token = authStorage.getAnyToken();
        if (token === authStorage.getToken(Role.ADMIN)) {
          user = authStorage.getUser(Role.ADMIN);
          authType = Role.ADMIN;
        } else if (token === authStorage.getToken(Role.PARTNER)) {
          user = authStorage.getUser(Role.PARTNER);
          authType = Role.PARTNER;
        } else if (token === authStorage.getToken(Role.CUSTOMER)) {
          user = authStorage.getUser(Role.CUSTOMER);
          authType = Role.CUSTOMER;
        }
      }
    ...
    handleLogout() {
      const currentRole = authStorage.getCurrentRoleFromPath(this.$route.path);
      authStorage.clearSession(currentRole);
