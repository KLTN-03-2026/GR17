  ...
  methods: {
    ...
    async loadPage() {
      ...
      try {
        ...
        this.errorMessage = error?.message || "Không thể tải dữ liệu hành trình.";
      } finally {
        this.loading = false;
      }
    },
    async attachExisting() {
      if (!this.attachForm.ma_dia_diem) {
        this.errorMessage = "Vui lòng chọn địa điểm cần gắn.";
        return;
      }
      ...
      try {
        ...
        this.successMessage = "Đã gắn địa điểm vào hành trình.";
        this.attachForm = attachFormDefault();
        this.itinerary = await fetchTourItinerary(this.maTour);
        this.mapEditRows();
      } catch (error) {
        this.errorMessage = error?.message || "Không thể gắn địa điểm vào hành trình.";
      } finally {
        this.saving = false;
      }
    },
    async createAndAttach() {
      if (!this.createForm.ten_dia_diem || !this.createForm.dia_chi) {
        this.errorMessage = "Vui lòng nhập tên và địa chỉ địa điểm.";
        return;
      }
      ...
      try {
        ...
        this.successMessage = "Đã tạo địa điểm mới và gắn vào hành trình.";
        ...
        this.errorMessage = error?.message || "Không thể tạo địa điểm mới cho hành trình.";
      } finally {
        this.saving = false;
      }
    },
    async updateItem(maChiTietTour) {
      ...
      try {
        ...
        this.successMessage = "Đã cập nhật hành trình.";
        this.itinerary = await fetchTourItinerary(this.maTour);
        this.mapEditRows();
      } catch (error) {
        this.errorMessage = error?.message || "Không thể cập nhật hành trình.";
      } finally {
        this.saving = false;
      }
    },
    async deleteItem(maChiTietTour) {
      ...
      try {
        ...
        this.successMessage = "Đã xóa địa điểm khỏi hành trình.";
        this.itinerary = await fetchTourItinerary(this.maTour);
        this.mapEditRows();
      } catch (error) {
        this.errorMessage = error?.message || "Không thể xóa điểm khỏi hành trình.";
      } finally {
        this.saving = false;
      }
    },
