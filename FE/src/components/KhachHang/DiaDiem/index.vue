      ...
      <section class="destination-content">
        ...
        <section class="toolbar">
          <form class="toolbar__search" @submit.prevent="timKiemDiaDiem">
            ...
            <input v-model.trim="boLoc.tim_kiem" type="text" placeholder="Bạn muốn đi đâu? (Ví dụ: Hạ Long, Sapa...)" :disabled="dang_tai">
          ...
          >
            ...
            <div class="destination-card__content">
              ...
              <div class="destination-card__bottom-row">
                <button type="button" class="destination-card__favorite-btn" :class="{ 'is-active': yeuThichMap[diaDiem.id] }" @click.stop="toggleYeuThich(diaDiem)" :title="yeuThichMap[diaDiem.id] ? 'Bỏ yêu thích' : 'Thêm vào danh sách yêu thích'">
...
export default {
  ...
  methods: {
    ...
    xacDinhKhuVuc(diaChi = "", ten = "") {
      ...
      if (/(hà giang|ha giang|sa pa|sapa|lào cai|lao cai|cao bằng|cao bang|mộc châu|moc chau)/.test(noiDung)) {
        return "vung-cao";
      }
      ...
      if (/(hà nội|ha noi|ninh bình|ninh binh|hải phòng|hai phong|quảng ninh|quang ninh|sapa)/.test(noiDung)) {
        return "mien-bac";
      }
    ...
    async taiDanhSachDiaDiem() {
      ...
      try {
        ...
        const res = await goiDuLieu(DIA_DIEM_API, { params });
        this.danh_sach_dia_diem = this.xuLyDanhSach(res.data).map((item, index) => this.chuanHoaDiaDiem(item, index));
      } catch (error) {
        this.thong_bao_loi = error?.phanHoi?.data?.message || "Không thể tải danh sách địa điểm.";
      } finally {
        this.dang_tai = false;
      }
    },
    ...
    yeuCauDangNhap(hanhDong) {
      const token = localStorage.getItem("token");
      if (token) {
        return true;
      }
      const xacNhan = window.confirm(`Bạn cần đăng nhập để ${hanhDong}. Chuyển tới trang đăng nhập ngay bây giờ?`);
      if (xacNhan) {
    ...
    async toggleYeuThich(diaDiem) {
      if (!this.yeuCauDangNhap("thêm địa điểm vào danh sách yêu thích")) {
        return;
      }
      
      if (!this.maKhachHang || !diaDiem?.id) {
        alert("Không đủ thông tin để thực hiện thao tác.");
        return;
      }
