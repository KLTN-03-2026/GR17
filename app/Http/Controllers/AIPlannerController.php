...
class AIPlannerController extends Controller
{
    ...
    public function suggestLocations(AIPlannerRequest $request)
    {
        ...
        try {
            $suggestions = $this->aiTourGuideService->suggestLocations(
                $diemDen,
                $soNgay,
                $nganSach,
                $soThichList,
    ...
    private function normalizeStringList($input): array
    {
        ...
        if (is_array($input)) {
            return array_values(array_filter(array_map('trim', array_map('strval', $input))));
        }
    ...
    {
        $context = "\n\nCAC YEU CAU BO SUNG KHAC:\n";
        $context .= "- So luong nguoi tham gia: $soNguoi nguoi.\n";

        if ($selectedLocations !== []) {
            $context .= "- BAT BUOC bao gom cac dia diem sau vao lich trinh: " . implode(', ', $selectedLocations) . ".\n";
        }

        if ($moTaChuyenDi !== '') {
            $context .= "- Ghi chu/Yeu cau rieng cua nguoi dung: $moTaChuyenDi\n";
        }
    ...
    {
        if (! empty($pexelsPhotos) && isset($pexelsPhotos[0]['src']['large2x'])) {
            return $pexelsPhotos[0]['src']['large2x'];
        }

        if (isset($itineraryData[0]['danhSachHoatDong'][0]['hinhanh'])) {
            return $itineraryData[0]['danhSachHoatDong'][0]['hinhanh'];
        }

        return 'https://picsum.photos/1200/600?random=' . time();
    }
    ...
    private function normalizeSuggestedLocations($suggestions): array
    {
        ...
        if (isset($suggestions['locations']) && is_array($suggestions['locations'])) {
            $suggestions = $suggestions['locations'];
        }

        return array_values(array_filter($suggestions, static fn ($item) => is_array($item) && isset($item['ten_dia_diem'])));
    }
