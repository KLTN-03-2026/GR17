<template>
  <nav class="admin-header">
    <div class="admin-header__left">
      ...
      <h2 class="admin-header__brand-sm">{{ headerTitle }}</h2>
    </div>
    ...
    <ul class="admin-header__nav">
      ...
      <li class="admin-header__item">
        ...
        <div class="admin-dropdown admin-dropdown--menu" :class="{ 'is-open': dropdownState.user }">
          ...
          <router-link class="admin-dropdown__menu-item" :to="profilePath">
            <i class="far fa-user"></i>
            <span>{{ profileLabel }}</span>
          </router-link>

          <router-link v-if="isAdmin" class="admin-dropdown__menu-item" to="/admin/tai-khoan">
            <i class="fas fa-user-shield"></i>
            <span>Quản lý tài khoản admin</span>
          </router-link>

          <router-link v-else-if="isPartner" class="admin-dropdown__menu-item" to="/doi-tac/dashboard">
            <i class="fas fa-briefcase"></i>
            <span>Không gian quản lý đối tác</span>
          </router-link>
...
export default {
  ...
  computed: {
    ...
    headerTitle() {
      return this.isPartner ? "Quản lý đối tác" : "Quản trị hệ thống";
    },
    displayName() {
      if (this.isPartner) {
        return (
    ...
    profilePath() {
      if (this.isPartner) return "/doi-tac/thong-tin-tai-khoan";
      return "/admin/ho-so";
    },
    profileLabel() {
      if (this.isPartner) return "Thông tin tài khoản";
  ...
  methods: {
    ...
    refreshSession() {
      ...
      this.authType = this.inferAuthType(this.userSnapshot);
    },
    toggleDropdown(key) {
    ...
    logout() {
      const wasPartner = this.isPartner;
  ...
  mounted() {
    ...
    window.addEventListener("partner-profile-updated", this.refreshSessionHandler);

    this.timer = setInterval(() => {
      this.now = new Date();
    }, 60000);

    if (globalNotifications.items.length === 0 && this.isAdmin) {
  ...
  beforeUnmount() {
    ...
    if (this.refreshSessionHandler) {
      ...
      window.removeEventListener("partner-profile-updated", this.refreshSessionHandler);
    }
    if (this.timer) {
