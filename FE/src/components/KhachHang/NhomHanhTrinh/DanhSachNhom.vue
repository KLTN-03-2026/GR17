              ...
              <tr v-for="group in filteredGroups" :key="group.maThanhVienNhom">
                ...
                <td>
                  <div class="row-actions">
                    ...
                    <button class="row-action" :class="{ 'row-action--disabled': group.roleValue !== 1 }" type="button" :disabled="group.roleValue !== 1" title="Đổi tên nhóm" @click="openEditDialog(group)">
                      <i class="fas fa-pen"></i>
                    </button>
                    <button class="row-action row-action--danger" :class="{ 'row-action--disabled': group.roleValue !== 1 }" type="button" :disabled="group.roleValue !== 1" title="Xóa nhóm" @click="openDeleteDialog(group)">
...
export default {
  ...
  methods: {
    ...
    openEditDialog(group) {
      if (group?.roleValue !== 1) {
        this.thongBaoLoi = "Chi nhom truong moi co the doi ten nhom.";
        return;
      }
      this.resetForm();
    ...
    openDeleteDialog(group) {
      if (group?.roleValue !== 1) {
        this.thongBaoLoi = "Chi nhom truong moi co the xoa nhom.";
        return;
      }
      this.nhomDangChon = group;
      this.clearMessages();
