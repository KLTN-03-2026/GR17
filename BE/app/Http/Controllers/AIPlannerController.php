...
class AIPlannerController extends Controller
{
                ...
                foreach ($aiResponse['lichTrinh'] as &$day) {
                    if (isset($day['danhSachHoatDong']) && is_array($day['danhSachHoatDong'])) {
                        foreach ($day['danhSachHoatDong'] as &$act) {
                            ...
                            // Nếu hoạt động thuộc tour mà chưa có hình, lấy hình của tour
                            if (!empty($act['ma_tour']) && empty($act['hinhanh'])) {
                                $tour = Tour::find($act['ma_tour']);
                                if ($tour) {
                                    $act['hinhanh'] = $tour->hinh_anh;
                                }
                            }
                        }
                    }
    ...
    public function saveItinerary(SaveItineraryRequest $request)
    {
        ...
        if (! $user instanceof KhachHang) {
            return response()->json([
                'success' => false,
                'message' => 'Chi khach hang moi co the luu ke hoach AI.',
            ], 403);
        }

        $maKhachHang = (string) $user->getKey();
        ...
        try {
            ...
            $keHoach = KeHoach::create([
                ...
                'so_nguoi' => $itineraryData['soNguoi'] ?? 1,
                'ngay_bat_dau' => $ngayBatDau,
                'ngay_ket_thuc' => $ngayKetThuc,
                'ngan_sach_du_kien' => (float) preg_replace('/\D/', '', $aiData['tongChiPhi'] ?? 0),
                'tong_chi_phi' => 0, // Sẽ tính toán lại sau khi thêm hoạt động
                'trang_thai' => 0,
                'nguon_tao' => 'ai',
                'du_lieu_ai' => $aiData,
            ]);
                ...
                foreach ($aiData['lichTrinh'] as $day) {
                    ...
                    if (isset($day['danhSachHoatDong']) && is_array($day['danhSachHoatDong'])) {
                        foreach ($day['danhSachHoatDong'] as $act) {
                            ...
                            $chiPhiHoatDong = 0;
                            if ($maDiaDiem) {
                                $locData = DiaDiem::find($maDiaDiem);
                                if ($locData) {
                                    $chiPhiHoatDong = (float) ($locData->gia_giao_dong ?: (preg_replace('/\D/', '', $act['gia'] ?? 0) ?: 0));
                                }
                            }
                            $tongChiPhiThucTe += $chiPhiHoatDong;
            ...
            // Cập nhật lại tổng chi phí thực tế cho kế hoạch
            $keHoach->update(['tong_chi_phi' => $tongChiPhiThucTe]);
