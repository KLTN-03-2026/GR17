...
class AITourGuideService
{
    ...
    public function isRetriableFailure(Throwable $exception): bool
    {
        ...
        foreach (['429', '503', 'quota', 'timeout', 'timed out', 'unavailable', 'overloaded', 'high demand', 'try again', 'gemini'] as $needle) {
...
YÊU CẦU ĐẶC BIỆT VỀ NGÂN SÁCH VÀ CHI PHÍ (QUAN TRỌNG):
...
2. ƯU TIÊN TUYỆT ĐỐI các địa điểm có sẵn trong danh sách "Địa điểm tại điểm đến" được cung cấp phía trên.
3. Nếu khách hàng đã CHỈ ĐỊNH CỨNG các Tour hoặc Địa điểm bắt buộc mà tổng chi phí của chúng vượt quá hoặc gần bằng với {nganSach}, bạn BẮT BUỘC PHẢI THÊM CÁC ĐỊA ĐIỂM/TOUR ĐÓ vào lịch trình. Đừng vì lý do vượt ngân sách mà tự ý loại bỏ mong muốn của khách.
4. CHIẾN LƯỢC BÙ TRỪ: Để hạn chế chi phí tăng thêm, hãy lấp đầy các khoảng thời gian rảnh rỗi còn lại bằng các hoạt động MIỄN PHÍ hoặc RẤT RẺ (công viên, bãi biển, dạo phố, chợ đêm...).
5. CẢNH BÁO VƯỢT NGÂN SÁCH: Nếu tổng chi phí dự kiến cao hơn mức {nganSach}, bạn bắt buộc phải tạo một thông điệp cảnh báo thân thiện vào trường `canhBao` (ví dụ: "Lịch trình này có thể vượt quá ngân sách dự kiến do bao gồm các Tour/địa điểm cao cấp mà bạn đã chọn. Chúng tôi đã bổ sung các hoạt động miễn phí để giúp bạn tối ưu chi phí."). Nếu nằm trong ngân sách, để trống hoặc ghi "Lịch trình tối ưu trong ngân sách của bạn.".
...
MẪU KẾT QUẢ ĐẦU RA (JSON):
{
  ...
  "tongChiPhi": "Tổng chi phí dự kiến (chỉ ghi con số, ví dụ: 5500000)",
  "canhBao": "Thông báo cảnh báo vượt ngân sách (nếu có)",
  "propossedTours": [
    {
      "ma_tour": "ID từ DB",
      "ten_tour": "Tên tour",
      ...
      "danhSachHoatDong": [
        {
          ...
          "gia": "Giá vé/chi phí dự kiến (con số)",
          "thoiLuong": "2 giờ"
        }
      ]
    }
