    ...
    async loadDepartureData() {
      ...
      try {
        ...
        if (tourResult.status === "rejected" || scheduleResult.status === "rejected") {
          this.errorMessage = "API lịch khởi hành chưa đầy đủ, trang đang hiển thị dữ liệu mẫu để bạn tiếp tục chỉnh giao diện.";
        }
      } catch (error) {
        this.tour = this.createFallbackTour();
        this.schedules = this.createFallbackSchedules();
        this.errorMessage = error.message || "Không thể tải dữ liệu lịch khởi hành.";
      }
    },
    addSchedule() {
