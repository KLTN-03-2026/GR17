<template>
  <div class="review-edit-page">
    <section class="editor-hero">
      <button class="back-button" type="button" @click="diDenChiTiet">
        <i class="fas fa-arrow-left"></i>
      </button>

      <div class="editor-hero__copy">
        <p class="editor-hero__eyebrow">Quản trị đánh giá</p>
        <h1>Chỉnh sửa đánh giá</h1>
        <p>Cập nhật số sao và nội dung đánh giá trong hệ thống quản trị.</p>
      </div>

      <div class="editor-hero__actions">
        <button class="ghost-button" type="button" @click="taiDanhGia" :disabled="dangTai || dangLuu">
          <i class="fas fa-rotate-right"></i>
          <span>{{ dangTai ? "Đang tải..." : "Tải lại" }}</span>
        </button>

        <router-link class="secondary-link" :to="`/admin/reviews/${reviewId}`">
    ...
    <section v-else-if="review" class="editor-grid">
      <form class="editor-card" @submit.prevent="luuDanhGia">
        <header class="editor-card__head">
          ...
          <div>
            ...
            <p>Dữ liệu gửi lên đúng chuẩn API <code>PUT /api/danh-gia-ke-hoach/{id}</code>.</p>
          </div>
        </header>

        <div class="field-grid">
          <label class="field">
            <span class="field__label">Mã đánh giá</span>
            <input :value="review.code" type="text" class="field__control field__control--readonly" readonly>
          </label>

          <label class="field">
            <span class="field__label">Khách hàng</span>
            <input :value="review.customerName" type="text" class="field__control field__control--readonly" readonly>
          </label>

          <label class="field field--full">
            <span class="field__label">Địa điểm</span>
            <input :value="review.locationName" type="text" class="field__control field__control--readonly" readonly>
          </label>

          <label class="field">
            <span class="field__label">Số sao</span>
            <select v-model.number="form.rating" class="field__control">
              <option v-for="option in ratingOptions" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </select>
            <small v-if="fieldErrors.so_sao" class="field__error">{{ fieldErrors.so_sao }}</small>
          </label>

          <label class="field">
            <span class="field__label">Hiển thị sao</span>
            <input :value="starPreview" type="text" class="field__control field__control--readonly" readonly>
          </label>

          <label class="field field--full">
            <span class="field__label">Nội dung đánh giá</span>
            <textarea
              v-model.trim="form.content"
              class="field__control field__control--textarea"
              rows="7"
              placeholder="Nhập nội dung đánh giá cần điều chỉnh"
            ></textarea>
            <small v-if="fieldErrors.noi_dung" class="field__error">{{ fieldErrors.noi_dung }}</small>
            <small class="field__hint">Có thể để trống nếu chỉ cần cập nhật số sao.</small>
          </label>
        </div>
        ...
        <div class="form-note">
          ...
          <div>
            <strong>Khuyến nghị nghiệp vụ</strong>
            <p>Ưu tiên chuẩn hóa chính tả, loại bỏ nội dung không phù hợp và giữ nguyên ý nghĩa phản hồi của khách hàng.</p>
          </div>
        </div>

        <footer class="editor-card__actions">
          <button class="ghost-button" type="button" @click="khoiPhucForm" :disabled="dangLuu || !originalForm">
            Khôi phục
          </button>
          <button class="danger-button" type="button" :disabled="dangLuu || isDeleting" @click="xoaDanhGia">
            <i v-if="isDeleting" class="fas fa-spinner fa-spin"></i>
            <i v-else class="fas fa-trash"></i>
            <span>{{ isDeleting ? "Đang xóa..." : "Xóa đánh giá" }}</span>
          </button>
      ...
      <aside class="summary-card">
        <div class="summary-card__profile">
          <div class="summary-card__avatar" :style="{ background: review.avatarGradient }">
            {{ review.customerInitials }}
          </div>
          <div>
            <p class="summary-card__eyebrow">Xem trước đánh giá</p>
            <h2>{{ review.customerName }}</h2>
            <div class="summary-card__tags">
              <span class="tag tag--slate">{{ review.code }}</span>
              <span :class="['tag', xacDinhLopSoSao(form.rating)]">
                {{ form.rating }}/5 sao
              </span>
            </div>
          </div>
    ...
    <section v-else class="empty-state">
      ...
      <router-link class="secondary-link" to="/admin/reviews">Quay về danh sách</router-link>
    </section>
  </div>
</template>
...
import { API_BASE, buildStars, mapReview, normalizeRecord, normalizeReviewId } from "./reviewAdminShared";
import { goiApi } from "../../../services/httpClient.js";
import { showConfirm } from "../../../services/appDialog";

export default {
  name: "ChinhSuaDanhGia",
  data() {
    return {
      dangTai: false,
      dangLuu: false,
  ...
  computed: {
    ...
    starPreview() {
      return buildStars(this.form.rating);
    },
    contentPreview() {
      return this.form.content || "Khách hàng chưa để lại nội dung đánh giá.";
    },
  },
  watch: {
    "$route.params.id": {
  ...
  methods: {
    ...
    kiemTraForm() {
      ...
      if (!Number.isInteger(this.form.rating) || this.form.rating < 1 || this.form.rating > 5) {
        errors.so_sao = "Số sao phải nằm trong khoảng từ 1 đến 5.";
      }

      if (this.form.content && this.form.content.length > 1000) {
        errors.noi_dung = "Nội dung đánh giá không được vượt quá 1000 ký tự.";
      }
    ...
    async taiDanhGia() {
      ...
      try {
        ...
        if (this.review) {
          this.ganDanhGiaVaoForm(this.review);
        }
      } catch (error) {
        this.review = null;
    ...
    async luuDanhGia() {
      ...
      try {
        const response = await goiApi(`${API_BASE}/danh-gia-ke-hoach/${encodeURIComponent(this.reviewId)}`, {
          ...
          body: JSON.stringify({
            so_sao: this.form.rating,
            noi_dung: this.form.content || null,
          }),
        });
        const payload = await response.json().catch(() => ({}));
        ...
        toaster.success("Đánh giá đã được cập nhật.");
      } catch (error) {
        this.saveMessage = {
          type: "error",
    ...
    async xoaDanhGia() {
      const confirmed = await showConfirm({
        ...
        message: `Xóa đánh giá DG ${this.reviewId} khỏi hệ thống?`,
        tone: "danger",
      ...
      try {
        ...
        toaster.success(`Đã xóa đánh giá DG ${this.reviewId}.`);
        ...
        toaster.error(error.message || "Không thể xóa đánh giá.");
      } finally {
        this.isDeleting = false;
      }
    },
    diDenChiTiet() {
      this.$router.push(`/admin/reviews/${this.reviewId}`);
    },
  },
};
</script>
