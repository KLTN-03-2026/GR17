...
export default {
  ...
  methods: {
    ...
    async fetchProfile() {
      if (!this.adminId) {
        await showAlert({ title: "Không tìm thấy phiên đăng nhập", message: "Không xác định được mã admin hiện tại trong bộ nhớ trình duyệt.", tone: "danger" });
        return;
      }
      ...
      try {
        ...
        await showAlert({ title: "Không thể tải hồ sơ", message: this.formatError(error, "Đã xảy ra lỗi khi tải thông tin tài khoản admin."), tone: "danger" });
      } finally {
    ...
    async saveProfile() {
      ...
      try {
        ...
        const current = parseUser() || {};
        saveUser({ ...current, ...this.profile });
        await showAlert({ title: "Đã cập nhật hồ sơ", message: response?.data?.message || "Thông tin cá nhân đã được lưu thành công.", tone: "success" });
      } catch (error) {
        await showAlert({ title: "Lưu hồ sơ thất bại", message: this.formatError(error, "Không thể cập nhật hồ sơ admin."), tone: "danger" });
      } finally {
    ...
    async changePassword() {
      ...
      if (this.passwordForm.new_password !== this.passwordForm.new_password_confirmation) {
        await showAlert({ title: "Xác nhận mật khẩu chưa khớp", message: "Mật khẩu mới và phần xác nhận phải trùng nhau.", tone: "warning" });
        return;
      }
      ...
      try {
        ...
        await showAlert({ title: "Đã đổi mật khẩu", message: response?.data?.message || "Mật khẩu quản trị đã được cập nhật thành công.", tone: "success" });
      } catch (error) {
        await showAlert({ title: "Đổi mật khẩu thất bại", message: this.formatError(error, "Không thể thay đổi mật khẩu cho tài khoản này."), tone: "danger" });
      } finally {
