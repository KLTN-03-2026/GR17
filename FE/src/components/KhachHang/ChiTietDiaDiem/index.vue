        ...
        <template v-if="dia_diem">
          ...
          <div class="main-layout-grid">
            ...
            <div class="main-column">
              ...
              <section class="info-section" v-if="dich_vu_doc.length > 0">
                <div class="section-header">
                  <h2>Ẩm thực & Giải trí</h2>
                  <a href="#">Xem tất cả <i class="fas fa-arrow-right"></i></a>
                </div>
                <div class="service-cards-vertical">
                  <!-- Vertical Cards -->
                  <article class="svc-vcard" v-for="dichVu in dich_vu_doc" :key="dichVu.id">
                    <div class="svc-vcard__cover" :style="{ backgroundImage: `url(${dichVu.hinh_anh})` }"></div>
                    <div class="svc-vcard__body">
                      <h3>{{ dichVu.ten }}</h3>
                      <div class="svc-vcard__meta">
                        <span>Nhà hàng cao cấp &bull; {{ ten_ngan }}</span>
                        <span class="rating-badge"><i class="fas fa-star"></i> {{ dichVu.diem_sao }}</span>
                      </div>
                    </div>
                  </article>
                </div>
              ...
              <section class="info-section" v-if="dich_vu_ngang.length > 0">
                <div class="section-header">
                  <h2>Khách sạn & Nghỉ dưỡng</h2>
                  <a href="#">Xem tất cả <i class="fas fa-arrow-right"></i></a>
                </div>
                <div class="service-cards-horizontal">
                  <!-- Horizontal Cards -->
                  <article class="svc-hcard" v-for="dichVu in dich_vu_ngang" :key="dichVu.id">
                    <div class="svc-hcard__cover" :style="{ backgroundImage: `url(${dichVu.hinh_anh})` }"></div>
                    <div class="svc-hcard__body">
                      <div class="svc-hcard__header">
                        <div>
                          <h3>{{ dichVu.ten }}</h3>
                          <div class="svc-hcard__tags">
                            <span class="tag-blue">5 sao</span>
                            <span class="svc-hcard__rating"><i class="fas fa-star"></i> {{ dichVu.diem_sao }} (2,100+)</span>
                          </div>
                        </div>
                        <div class="svc-hcard__price">
                          <strong>{{ dichVu.gia_hien_thi }}</strong>
                          <span>/đêm</span>
                        </div>
                      </div>
                      <p class="svc-hcard__desc">{{ dichVu.mo_ta }}</p>
                      <div class="svc-hcard__amenities">
                        <span><i class="fas fa-swimming-pool"></i> Hồ bơi</span>
                        <span><i class="fas fa-spa"></i> Spa</span>
                        <span><i class="fas fa-utensils"></i> Ăn sáng free</span>
                      </div>
                    </div>
                  </article>
                </div>
...
const API_DICH_VU = "/api/dich-vu-dia-diem/location";
...
export default {
  ...
  computed: {
    ...
    dich_vu_doc() {
      // Top 2 elements for vertical cards (Ăn uống giả định)
      return this.danh_sach_dich_vu.slice(0, 2);
    },
    dich_vu_ngang() {
      // Remaining elements for horizontal cards (Khách sạn giả định)
      return this.danh_sach_dich_vu.slice(2, 4);
    },
    thong_ke_danh_gia() {
  ...
  methods: {
    ...
    chuanHoaDichVu(item = {}, index = 0) {
      const gia = this.layGiaTriDauTien(item, ["gia", "Gia"], 0);
      const giaFormatted = (Number(gia) > 0) ? `${Number(gia).toLocaleString("vi-VN")}đ` : "Liên hệ";
      return {
        id: this.layGiaTriDauTien(item, ["ma_dich_vu", "Ma_dich_vu", "id", "Id"], index + 1),
        ten: this.layGiaTriDauTien(item, ["ten_dich_vu", "Ten_dich_vu"], "Dịch vụ đẳng cấp"),
        mo_ta: "Khu nghỉ dưỡng quy mô cung cấp trải nghiệm trọn vẹn với các tiêu chuẩn phục vụ quốc tế chuyên nghiệp.",
        gia_hien_thi: giaFormatted,
        diem_sao: (4.5 + (index % 4) * 0.1).toFixed(1),
        hinh_anh: this.dia_diem?.hinh_anh || HINH_MAC_DINH,
      };
    },
    async taiChiTietDiaDiem() {
      ...
      this.thong_bao_loi = "";
      try {
        const [chiTietRes, dichVuRes, danhGiaRes] = await Promise.allSettled([
          fetchDuLieu(`${API_DIA_DIEM}/${this.ma_dia_diem}`),
          fetchDuLieu(`${API_DICH_VU}/${this.ma_dia_diem}`),
        ...
        if (dichVuRes.status === "fulfilled") {
          this.danh_sach_dich_vu = this.xuLyDanhSach(dichVuRes.value.data).map((item, i) => this.chuanHoaDichVu(item, i));
          // Nếu ko có dữ liệu giả lập cho đẹp giống mockup
          if (this.danh_sach_dich_vu.length === 0) {
             this.danh_sach_dich_vu = [
               this.chuanHoaDichVu({ ten_dich_vu: "Nhà hàng Cua Vàng" }, 0),
               this.chuanHoaDichVu({ ten_dich_vu: "Sky Bar Lounge" }, 1),
               this.chuanHoaDichVu({ ten_dich_vu: "Vinpearl Resort & Spa", gia: 2450000 }, 2)
             ];
             // Set different images for aesthetics
             this.danh_sach_dich_vu[0].hinh_anh = "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=800";
             this.danh_sach_dich_vu[1].hinh_anh = "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=800";
             this.danh_sach_dich_vu[2].hinh_anh = "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800";
             this.danh_sach_dich_vu[2].mo_ta = "Khu nghỉ dưỡng biệt lập trên đảo Rều với tầm nhìn 360 độ ra Vịnh Hạ Long hùng vĩ, tiện nghi đẳng cấp.";
          }
        }
        ...
        this.thong_bao_loi = "Không thể tải chi tiết địa điểm. " + (error.message || "");
      } finally {
        this.dang_tai = false;
      }
    },
    goToPlan() {
      ...
      if (!token) {
        if(window.confirm("Bạn cần đăng nhập để thêm vào hành trình. Chuyển tới đăng nhập?")) this.$router.push("/dang-nhap");
        return;
      }
    ...
    async addCurrentDestinationToFavorites() {
      const token = localStorage.getItem("token");
      if (!token) {
        if(window.confirm("Bạn cần đăng nhập để lưu yêu thích. Chuyển tới đăng nhập?")) this.$router.push("/dang-nhap");
        return;
      }
      this.actionMessage = "Đã lưu địa điểm vào danh sách yêu thích cá nhân.";
...
/* DỊCH VỤ NGANG (HORIZONTAL CARDS) */
