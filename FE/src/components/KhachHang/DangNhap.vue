...
export default {
  ...
  methods: {
    ...
    async taiKhachHangTheoEmail(email) {
      const phanHoi = await goiApi(`${API_BASE}/khach-hang`, {
        headers: {
      ...
      if (!phanHoi.ok) {
        throw new Error(data.message || this.copy.messages.khachHangTaiThatBai);
      }
    ...
    async dangNhapKhachHang() {
      ...
      const customer = data?.user || (await this.taiKhachHangTheoEmail(this.form.email));
      if (!customer) {
        return {
          ok: false,
          status: phanHoi.status,
