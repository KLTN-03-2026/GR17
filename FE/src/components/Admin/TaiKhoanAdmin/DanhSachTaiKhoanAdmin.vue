  ...
  <div class="admin-account-page">
    <section class="hero">
      <div>
        ...
        <p class="subtitle">Quản lý danh sách quản trị viên, phân vai trò, khóa mở tài khoản và kiểm soát thông tin đăng nhập nội bộ.</p>
      </div>
              ...
              <td>
                <div class="actions-inline justify-center">
                  ...
                  <button
                    class="icon-btn"
                    type="button"
                    :title="isBlocked(admin) ? 'Mở khóa tài khoản' : 'Khóa tài khoản'"
  ...
  methods: {
      ...
      try {
        ...
        if (error?.response?.status === 404) {
          ...
          await showAlert({
            ...
            message: this.formatError(error, "Đã xảy ra lỗi khi tải danh sách quản trị viên."),
            tone: "danger",
          });
        }
      } finally {
    ...
    async saveAdmin() {
      ...
      try {
        ...
        if (this.form.Ma_admin) {
          ...
          await showAlert({ title: "Đã cập nhật tài khoản", message: response?.data?.message || "Thông tin quản trị viên đã được cập nhật.", tone: "success" });
        } else {
          ...
          await showAlert({ title: "Đã tạo tài khoản", message: response?.data?.message || "Tài khoản admin mới đã được tạo thành công.", tone: "success" });
        }
        this.closeModal();
      } catch (error) {
        await showAlert({ title: "Lưu tài khoản thất bại", message: this.formatError(error, "Không thể lưu thông tin tài khoản admin."), tone: "danger" });
      } finally {
        this.dangLuu = false;
      }
    },
    async toggleStatus(admin) {
      ...
      if (adminId === this.currentAdminId) {
        await showAlert({ title: "Không thể tự khóa tài khoản", message: "Bạn đang đăng nhập bằng tài khoản này. Hãy dùng một tài khoản quản trị khác nếu cần khóa nó.", tone: "warning" });
        return;
      }
      ...
      try {
        ...
        await showAlert({ title: "Đã cập nhật trạng thái", message: response?.data?.message || "Trạng thái tài khoản admin đã được cập nhật.", tone: "success" });
      } catch (error) {
        await showAlert({ title: "Không thể cập nhật trạng thái", message: this.formatError(error, "Đã xảy ra lỗi khi thay đổi trạng thái tài khoản admin."), tone: "danger" });
      } finally {
    ...
    async deleteAdmin(admin) {
      const adminId = this.adminKey(admin);
      if (adminId === this.currentAdminId) {
        await showAlert({ title: "Không thể tự xóa tài khoản", message: "Bạn đang đăng nhập bằng tài khoản này. Hãy dùng một tài khoản quản trị khác nếu cần xóa nó.", tone: "warning" });
        return;
      }
      ...
      try {
        ...
        await showAlert({ title: "Đã xóa tài khoản", message: response?.data?.message || "Tài khoản admin đã được xóa thành công.", tone: "success" });
      } catch (error) {
        await showAlert({ title: "Xóa tài khoản thất bại", message: this.formatError(error, "Không thể xóa tài khoản admin này."), tone: "danger" });
      } finally {
        this.popBusy(adminId);
      }
    },
...
<style scoped>
.admin-account-page { min-height: 100%; padding: 2rem 2.25rem 2.5rem; background: radial-gradient(circle at top right, rgba(37,99,235,.08), transparent 24%), linear-gradient(180deg, #f7f9fe 0%, #eef3fb 100%); }
.hero, .hero-actions, .toolbar, .card-head, .modal-actions { display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
...
.stats-grid, .form-grid { display: grid; gap: 1rem; }
...
.stats-card:nth-child(3) i { background: linear-gradient(135deg, #ef4444, #dc2626); }
.stats-card:nth-child(4) i { background: linear-gradient(135deg, #7c3aed, #6d28d9); }
.stats-card span { color: #64748b; font-size: .88rem; }
...
.admin-table th, .admin-table td { padding: 1rem .85rem; border-bottom: 1px solid #eef2f7; vertical-align: middle; text-align: left; }
.admin-table th { color: #64748b; font-size: .8rem; text-transform: uppercase; letter-spacing: .08em; }
.admin-table td strong, .admin-table td small { display: block; }
.admin-table td strong { color: #0f172a; font-size: .97rem; }
.admin-table td small { margin-top: .25rem; color: #64748b; font-size: .86rem; }
...
.empty-state { min-height: 240px; display: grid; place-items: center; gap: .65rem; text-align: center; border: 1px dashed #dbe4f0; border-radius: 1rem; color: #64748b; }
...
.modal-card { width: min(100%, 760px); }
...
@media (max-width: 768px) {
  .admin-account-page { padding: 1rem; }
  .hero, .hero-actions, .toolbar, .card-head, .modal-actions { flex-direction: column; align-items: stretch; }
