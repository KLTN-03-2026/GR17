...
export default {
  ...
  methods: {
    ...
    savePartnerSession(token, doiTac) {
      authStorage.saveSession(Role.PARTNER, { token, user: doiTac });
    },
    async handleLogin() {
      ...
      try {
        ...
        const redirectPath = String(this.$route.query.redirect || "/doi-tac/dashboard");
    ...
    redirectIfLoggedIn() {
      const partnerToken = authStorage.getToken(Role.PARTNER);
      if (partnerToken) {
        const redirectPath = String(this.$route.query.redirect || "/doi-tac/dashboard");
