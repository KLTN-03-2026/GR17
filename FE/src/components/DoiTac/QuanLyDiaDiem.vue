<template>
  ...
  >
          ...
          <tr v-for="location in filteredLocations" :key="location.ma_dia_diem">
            ...
            <td class="actions">
              ...
              <button
                type="button"
                class="action-danger"
                :disabled="deletingLocationId === location.ma_dia_diem"
    ...
    <DichVuDiaDiemModal 
      v-if="showDichVuModal" 
      :location="selectedLocationForDichVu" 
      @close="closeDichVuModal" 
    />
  </PartnerShell>
</template>
  ...
  methods: {
    ...
    async loadPage() {
      ...
      try {
        ...
        this.errorMessage = error?.message || "Không thể tải dữ liệu quản lý địa điểm.";
      } finally {
        this.loading = false;
      }
    },
