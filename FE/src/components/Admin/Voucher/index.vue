  ...
  <div class="container-fluid py-4">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">
        <i class="fas fa-ticket-alt text-primary mr-2"></i> Quản lý Mã Giảm Giá
      </h1>
...
export default {
  ...
  setup() {
    ...
    const fetchVouchers = async () => {
      loading.value = true;
      try {
        const response = await goiApi('/api/admin/vouchers');
    ...
    const saveVoucher = async () => {
      ...
      try {
        const url = isEditing.value 
          ? `/api/admin/vouchers/${editingId.value}`
          : '/api/admin/vouchers';
    ...
    const deleteVoucher = async (id) => {
      ...
      try {
        const response = await goiApi(`/api/admin/vouchers/${id}`, {
          method: 'DELETE'
        });
        const data = await response.json();
        if (data.success) {
    ...
    const toggleStatus = async (id) => {
      try {
        const response = await goiApi(`/api/admin/vouchers/${id}/status`, {
          method: 'PATCH'
        });
        const data = await response.json();
        if (data.success) {
