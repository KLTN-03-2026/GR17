  ...
  methods: {
    ...
    async duyetDiaDiem(diaDiem) {
      ...
      try {
        ...
        toaster.success(`Địa điểm ${diaDiem.ten} đã được duyệt.`);
      } catch (error) {
        toaster.error(this.layThongBaoLoi(error, "Không thể duyệt địa điểm đối tác."));
      } finally {
        this.dangXuLyDuyet = "";
      }
    },
    async tuChoiDiaDiem(diaDiem) {
      ...
      try {
        ...
        await this.taiDanhSachDiaDiem(this.phanTrang.trangHienTai);
        toaster.success(`Địa điểm ${diaDiem.ten} đã được chuyển sang trạng thái chưa duyệt.`);
      } catch (error) {
        toaster.error(this.layThongBaoLoi(error, "Không thể từ chối địa điểm đối tác."));
      } finally {
        this.dangXuLyDuyet = "";
      }
    },
